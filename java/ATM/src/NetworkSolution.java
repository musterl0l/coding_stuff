 ...
 * 
 */
public class NetworkSolution implements INetwork {
	// -- attributes --
	...
	public List<Edge> findMinCut() {
		...
		// calculates maxflow and puts the residual graph into the attribute "residualGraph"
		this.fordFulkerson();

		// List to store Edges which should be cut
		cutEdges = new Vector<Edge>(); //TODO: change to linkedlist for simplicity
		
		// find the reachable nodes in residual graph
		// bfs colors all reachable nodes black -> they are in the set of the
		// source nodes
		List<Node> reachableNodes = residualGraph.findReachableNodes(source.id);
		// now coloring the nodes in the network like the reachable nodes in
		// the residual graph
		for (Node cur : nodes.values()) {
			this.nodes.get(cur.getID()).status = 0;
		}
		for (Node cur : reachableNodes) {
			this.nodes.get(cur.getID()).status = 2;
		}
		//Cut any edge that connects nodes of status 2 with 0:
		for (Node cur : nodes.values()) {
			LinkedList<Edge> edges = cur.getIncidentEdges();
			for (Edge e : edges) {
				if (e.getEndnode().status==0 && cur.status==2) {
					
					e.status = 5; // mark cut Edge red
					cutEdges.add(e);
				}
				// nodes have different colors, so they are in different sets
				// at this edge is no positive flow available
			}
		}
		return cutEdges;
	}
	...
	public INetwork initializeResidualGraph() {
		...
		NetworkSolution residualGraph = new NetworkSolution();
		Edge reverseEdge;
		// adding nodes
