...
//			if (neighbor != null) {n.addEdge(neighbor, 1);}
	...
	public void populateAStar(CellNode startCNode, CellNode targetCNode) {
		...
		// priority queue for candidate CellNodes with the size of the cellnodes hashset and the Comparator h with the heuristic
		PriorityQueue<CellNode> distanceQueue = new PriorityQueue<CellNode>(cellnodes.size(), h);
		...
		while (!distanceQueue.isEmpty()) {
			...
			for (Edge incidentEdge : current.getIncidentEdges()) {
				...
				// outgoing edge,
				// i.e. e.next
				Node n = incidentEdge.endnode;
				CellNode neighbor = this.getCellNode(n);
	...
	/**
	 * Calculates a List of Node indices that describe the shortest path from
	...
	public List<Node> getShortestPathAStar(Node startNode, Node targetNode) {
		...
		while (reversePathNode != null &&
			...
			l.addFirst(reversePathNode);
			buf = reversePathNode.predecessor;
			reversePathNode = this.getCellNode(buf);
		}
