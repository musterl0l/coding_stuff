...
public class Database {
	...
	public Pair< String, IMatcher> lookup(ISignal query) {
		...
		*/
		...
		IMatcher match = null; 	//best match
		IMatcher cur = null;	//current match
		Pair<String, ISignal> best = null; 
		
		for (Pair<String, ISignal> p : this.database){
			//set best match
			if (match == null){
				match = compare(p.getRight(), query);
				best = p;
			}else{
				cur = compare(p.getRight(), query);
				//compare distance witch best match so far
				if(match.getDistance() > cur.getDistance()){
					match = cur;
					best = p;
				}
			}
		...
		return new Pair<String, IMatcher>(best.getLeft(), match);
	}
